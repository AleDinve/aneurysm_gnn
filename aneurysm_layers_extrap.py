    ...
    for sub_ind in sublists(indicators):
        if len(sub_ind)==1: #only pairs of indicators, or the three of them
            for num_layers in num_layers_list:
                ...
                log_path = 'extrap/' + '_'.join([abl_type]+sub_ind)
...
if __name__ == '__main__':
    ...
    parser.add_argument("--conv", help="type(s) of Graph Convolutional Layer (passed as list string)", type=str, default='[gtr]')
    parser.add_argument("--epochs", help="extra features", type=int, default=5000)
    args = parser.parse_args()
    conv_types = args.conv.strip('][').split(', ')
    print(conv_types[0])
